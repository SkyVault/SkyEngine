...
#ifndef NEW_COMP
#define NEW_COMP(C, ...) memcpy(malloc(sizeof(C)),&(C const){__VA_ARGS__},sizeof(C));
#endif//NEW_COMP
...
#define add_comp(world, ent, comp, ...)                                 \
    { int place = find_free_comp(&world->components[CAT(CT_, comp)]);   \
        ...
        world->components[CAT(CT_, comp)].list[place] = (void*)NEW_COMP(comp,__VA_ARGS__); } \

#define add_comp_obj(world, ent, comp, init)\
    { int place = find_free_comp(&world->components[CAT(CT_, comp)]);   \
        ent->component_indexes[CAT(CT_, comp)] = place;                 \
        world->components[CAT(CT_, comp)].list[place] = (void*)malloc(sizeof(comp)); \
        comp* _c = get_comp(world, ent, comp);                          \
        *_c = (init); }                                                 \

#define get_comp(world, ent, comp)                                      \
    ((comp*)world->components[CAT(CT_, comp)].list[ent->component_indexes[CAT(CT_, comp)]])
