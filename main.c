...
void update_and_render_menu_scene(MainMenuState *state, Game *game,
  ...
  float rot = 360 - ((ease(TWEEN_BOUNCEOUT, timer) * 180.0f) - 180) - (45 / 2);
  ...
  while (!WindowShouldClose() && game->state == STATE_RUNNING) {
    ...
    if (assets->shaders_hotload[SHADER_PHONG_LIGHTING].just_hotloaded) {
      ...
      for (int i = 0; i < assets->num_lights; i++) {
        ...
        Light light =
            CreateLight(LIGHT_POINT, self.position, self.target, self.color,
                        assets->shaders[SHADER_PHONG_LIGHTING]);
        UpdateLightValues(assets->shaders[SHADER_PHONG_LIGHTING], light);
    ...
    case SCENE_EDIT: {
      update_and_render_game_scene_with_editor(game, ecs, particle_sys, map,
                                               gfx, editor);
      break;
    }

    case SCENE_MAIN_MENU: {
      update_and_render_menu_scene(menu_state, game, ecs, particle_sys, map,
                                   gfx);
      break;
    }
    }
